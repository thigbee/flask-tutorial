...
. Similarly, client.post() makes a POST request, converting the data dict into 
form data.

To test that the page renders successfully, a simple request is made and checked 
for a 200 OK status_code. If rendering failed, Flask would return a 500 Internal 
Server Error code.
...
to render on the page, check that it’s in data. Bytes must be compared to bytes. 
...
The tests for the login view are very similar to those for register. Rather than 
testing the data in the database, session should have user_id set after logging in.
...
Logout:
Testing logout is the opposite of login. session should not contain user_id after 
logging out.
