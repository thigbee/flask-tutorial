'''
Imports and connects an sqlite3 database
...
the request. The connection is stored and reused instead of
creating a new connection if get_db is called a second time in 
the same request.
...
your code. get_db will be called when the application has been 
...
location is when deploying the application later. get_db returns 
a database connection, which is used to execute the commands read 
from the file.
...
You can read Command Line Interface to learn more about writing 
commands.
